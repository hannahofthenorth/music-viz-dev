...
export function fractal(ctx, len, x0, y0, angle, overallAngle) {
  // angle in [0,360]
  branch(ctx, len, 1, x0, y0, angle, overallAngle)
  branchMirror(ctx, len, 1, x0, y0, angle, overallAngle)
  return ctx
}
...
export function branch(ctx, len, counter, x0, y0, angle, overallAngle) {
  ...
  //   return
  // }
  branch(ctx, 0.75*len, counter+1, len*Math.sin(overallAngle), len*Math.cos(overallAngle), angle, overallAngle)
  branch(ctx, 0.75*len, counter+1, len*Math.sin(overallAngle), len*Math.cos(overallAngle), -angle, overallAngle)
  ctx.restore()
}
export function branchMirror(ctx, len, counter, x0, y0, angle, overallAngle) {
  if (counter==0){
    ctx.save()
    ctx.moveTo(x0, y0)
    ctx.rotate(Math.PI/3)
  ...
  branchMirror(ctx, 0.75*len, counter+1, len*Math.sin(overallAngle), -len*Math.cos(overallAngle), angle, overallAngle)
  branchMirror(ctx, 0.75*len, counter+1, len*Math.sin(overallAngle), -len*Math.cos(overallAngle), -angle, overallAngle)
  ctx.restore()
}