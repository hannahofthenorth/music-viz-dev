...
import { fireshrine } from '../util/color_themes'

export default class RotatingDots extends Visualizer {
  constructor () {
    super({ volumeSmoothing: 10 })
    this.theme = fireshrine
    this.counter = 1
    this.rotation = 20
    this.height = 256
    this.section = 0
  ...
  paint ({ ctx, height, width, now }) {
    ...
    ctx.fillStyle = 'rgba(0, 0, 0, .5)' //Fill the background black at alpha 0.05 for fade effect
    ...
    fractal(ctx, 175, width/2, height/2, this.rotation, this.overallRotation, 6)
    ctx.stroke()
  }
}